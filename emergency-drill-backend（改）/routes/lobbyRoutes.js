...
router.post('/create', async (req, res) => {
    ...
    // 从环境变量中获取服务器IP，这个变量在 server.js 启动时被设置
    // 这确保了即使在局域网内，也能获取到正确的IP地址
    const serverIp = process.env.SERVER_IP || '127.0.0.1'; // 默认回退到localhost，确保server.js中会设置局域网IP
    ...
    // 【关键修改点】：生成的 joinUrl 包含服务器实际检测到的IP，供学生扫描
    // 这样前端就不需要自己去猜测IP，而是直接使用后端提供的准确URL
